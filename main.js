...
function gotPoses(results) {
 if (results.length > 0) {
   ...
   rightWristScore = Math.round(results[0].pose.keypoints[10].score * 10000);
}
}

function startzegame() {
  game_stat = 'true';
  document.getElementById('status').innerHTML = 'Game Loaded.';
  document.getElementById('canvas').display = 'fixed';
}
